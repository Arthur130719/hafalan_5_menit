...
recordBtn.addEventListener('click', function () {
    if (!recognition) {
        alert('Browser Anda tidak mendukung fitur pengenalan suara. Silakan gunakan browser Chrome.');
        return;
    }
    try {
        finalTranscript = '';
        transcriptBox.classList.remove('hidden');
        transcript.textContent = 'Mendengarkan... Silakan ucapkan hafalan Anda.';
        ...
        alert('Terjadi kesalahan saat memulai mikrofon: ' + error.message);
    }
});
...
stopBtn.addEventListener('click', function () {
    if (isRecognizing && recognition) {
        ...
        transcript.textContent = finalTranscript || 'Memproses...';
    }
});
...
if (recognition) {
    recognition.onresult = (event) => {
        ...
        finalTranscript = finalTrans;
        transcript.textContent = finalTrans + interimTranscript;
    };
    ...
    recognition.onend = () => {
        ...
        if (recognizedText) {
            ...
            transcript.textContent = '(Tidak ada ucapan terdeteksi)';
            retryBtn.classList.remove('hidden');
            recordBtn.classList.remove('hidden');
        }
    };
    ...
    recognition.onerror = (event) => {
        ...
        if (event.error === 'no-speech') {
            transcript.textContent = 'Tidak ada suara terdeteksi. Silakan coba lagi.';
        } else {
...
function compareTexts(original, recognized) {
    const originalLower = removePunctuation(original.toLowerCase().trim());
    const recognizedLower = removePunctuation(recognized.toLowerCase().trim());
...
finishBtn.addEventListener('click', function () {
    ...
    finalResult.innerHTML = `
        <strong>Hasil Akhir Sesi Ini</strong><br>
        Teks Asli: \"${text}\"<br><br>
        Total Percobaan: ${totalAttempts}<br>
        Benar: ${correctCount}/${totalAttempts}<br>
        Tingkat Akurasi: ${avgAccuracy}%<br><br>
        ${avgAccuracy >= 85 ? 'Sangat baik! Anda sudah berhasil menghafalnya.' : avgAccuracy >= 70 ? 'Bagus! Terus tingkatkan.' : 'Terus berlatih, Anda pasti bisa!'}
    `;
});
